...
@register_action('add_video')
@register_command('add_video')
def add_video() -> bool:
    trace_in()
    gateway = get_gateway()
    ...
    if not is_error():
        ...
        if not Path(file_path).exists():
            warn(f"Video file not found: {file_path}")
            report_error("action", f"Video file not found: {file_path}")
        elif not os.access(file_path, os.R_OK):
            warn(f"Video file not readable: {file_path}")
            report_error("action", f"Video file not readable: {file_path}")
    if not is_error():
        try:
    ...
    if not is_error() and parent_page is not None:
        ...
        log(f"Adding video '{caption}' to page {target_page_id}")
        new_video_id = parent_page.add_video(file_path, caption)
        video_created = not is_error() and new_video_id is not None
        if not video_created:
            warn("Video creation failed")
            report_error("action", "Video creation failed")
    updated_page = None
    if not is_error():
        log(f"Reloading parent page {target_page_id} to show new video")
    ...
    if not is_error() and updated_page is not None:
        ...
        log(f"Successfully added video {new_video_id} to page {target_page_id}: {page_name}")
    trace_out()
    return not is_error()
