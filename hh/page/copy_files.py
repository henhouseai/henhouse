...
@register_action("copy_files")
@register_command("copy_files")
def copy_files() -> bool:
    """
    Copy files from a source page to a target page.
    ...
    if not is_error() and not file_ids:
        warn("No files to copy")
        report_error("action", "No files to copy")
