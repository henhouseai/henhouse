...
@register_action('copy_video')
@register_command('copy_video')
def copy_video() -> bool:
    trace_in()
    gateway = get_gateway()
    ...
    if not is_error():
        ...
        try:
            ...
            warn(f"Invalid target page ID: {target_page_arg} or video ID: {video_id_arg}")
            report_error("action", "Target page ID and video ID must be numbers")
    ...
    if not is_error() and page is not None:
        log(f"Copying video {video_id} to target page {target_page_id}")
        success = page.copy_media_items("video", [video_id], target_rank=rank_int)
        if not success:
            warn(f"Failed to copy video {video_id} to target page {target_page_id}")
            report_error("action", f"Failed to copy video {video_id} to target page {target_page_id}")
    ...
    if not is_error() and updated_page is not None:
        ...
        if rank_int is not None:
            ...
            log(f"Successfully copied video {video_id} to target page {target_page_id} and set rank to {rank_int}")
        else:
            response_data.update({
                "rank_operation": "skipped"
            })
            log(f"Successfully copied video {video_id} to target page {target_page_id}")
