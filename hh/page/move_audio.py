...
@register_action('move_audio')
@register_command('move_audio')
def move_audio() -> bool:
    trace_in()
    gateway = get_gateway()
    ...
    if not is_error():
        ...
        try:
            ...
            warn(f"Invalid IDs provided. Source: {source_page_arg}, Audio: {audio_id_arg}, Target: {target_page_arg}")
    ...
    if not is_error() and source_page is not None:
        if source_rank:
            ...
            # Auto-detect old_rank if not provided
            log(f"Source rank not provided, checking for audio instances in source page {source_page_id}")
            current_audio = source_page.get_audio_data()
            audio_instances = [aud for aud in current_audio if aud['id'] == audio_id]
            
            if len(audio_instances) == 0:
                warn(f"Audio {audio_id} not found in source page {source_page_id}")
                report_error("action", f"Audio {audio_id} not found in source page {source_page_id}")
            elif len(audio_instances) > 1:
                warn(f"Audio {audio_id} appears {len(audio_instances)} times in source page {source_page_id}. Old rank must be specified.")
                report_error("action", f"Audio {audio_id} appears {len(audio_instances)} times in source page {source_page_id}. Old rank must be specified.")
            else:
                # Use the single instance's rank
                source_rank_int = audio_instances[0]['audio_rank']
                log(f"Using detected source rank {source_rank_int} for audio {audio_id}")
    ...
    if not is_error() and target_page is not None and source_rank_int is not None:
        log(f"Moving audio {audio_id} from page {source_page_id} rank {source_rank_int} to page {target_page_id}")
        # Create audio instance structure for move_audios
        audio_instances = [{
            'audio_id': audio_id,
        ...
        if not success:
            warn(f"Failed to move audio {audio_id} from page {source_page_id} to page {target_page_id}")
            report_error("action", f"Failed to move audio {audio_id} from page {source_page_id} to page {target_page_id}")
    ...
    if not is_error() and updated_page is not None:
        ...
        response_data.update({
            "moved_audio_id": audio_id,
            "source_page_id": source_page_id,
            "source_rank": source_rank_int,
            "operation": "move_audio"
        })
        ...
        if target_rank_int is not None:
            ...
            log(f"Successfully moved audio {audio_id} from page {source_page_id} rank {source_rank_int} to page {target_page_id} rank {target_rank_int}")
        else:
            response_data.update({
                "rank_operation": "skipped"
            })
            log(f"Successfully moved audio {audio_id} from page {source_page_id} rank {source_rank_int} to page {target_page_id}")
