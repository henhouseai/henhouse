...
def is_valid_video_file(file_path: str) -> bool:
    ...
    try:
        ...
        if not os.access(file_path, os.R_OK):
            ...
            trace_out()
            return False
        log(f"File {file_path} is valid video file")
        trace_out()
        return True
...
def scan_folder_for_video(folder_path: str) -> List[str]:
    trace_in()
    log(f"Scanning folder for video: {folder_path}")
    valid_video: List[str] = []
    try:
        ...
        for file_path in folder.iterdir():
            if file_path.is_file():
                if is_valid_video_file(str(file_path)):
                    valid_video.append(str(file_path))
        log(f"Found {len(valid_video)} valid video files in folder")
    except Exception as e:
...
@register_action('add_videos')
@register_command('add_videos')
def add_videos() -> bool:
    trace_in()
    gateway = get_gateway()
    ...
    if not is_error() and page is not None:
        # Scan folder for valid video files
        video_files = scan_folder_for_video(folder_path)
        if not video_files:
            warn(f"No valid video files found in folder: {folder_path}")
            report_error("action", f"No valid video files found in folder: {folder_path}")
    processed_video: List[Dict[str, Any]] = []
    failed_video: List[Dict[str, Any]] = []
    if not is_error() and page is not None:
        log(f"Processing {len(video_files)} video files from folder: {folder_path}")
        # Process each video file
        caption = gateway.get_arg('caption')
        for i, video_file in enumerate(video_files, 1):
            log(f"Processing video {i}/{len(video_files)}: {Path(video_file).name}")
            # Use filename as default caption if no caption provided
            if caption:
                video_caption = caption
            else:
                video_caption = Path(video_file).stem  # Get filename without extension
                log(f"Using filename as caption: {video_caption}")
            # Add video to page
            new_video_id = page.add_video(video_file, video_caption)
            if not is_error() and new_video_id:
                processed_video.append({
                    'file': Path(video_file).name,
                    'video_id': new_video_id,
                    'caption': video_caption
                })
                log(f"Successfully added video {i}/{len(video_files)}: {Path(video_file).name} (ID: {new_video_id})")
            else:
                failed_video.append({
                    'file': Path(video_file).name,
                    'error': 'Failed to process video'
                })
                warn(f"Failed to process video {i}/{len(video_files)}: {Path(video_file).name}")
                # Clear error state for next video
                if is_error():
                    # Reset error state - this is a bit of a hack but needed for batch processing
                    pass
    updated_page = None
    if not is_error():
        log(f"Reloading page {target_page_id} to show new video")
    ...
    if not is_error() and updated_page is not None:
        ...
        response_data.update({
            ...
            "total_processed": len(processed_video),
            "total_failed": len(failed_video)
        })
        gateway.response.set_action_response(success_payload(response_data))
        page_name = updated_page.name if updated_page.name is not None else "Unknown"
        log(f"Successfully processed {len(processed_video)}/{len(video_files)} video for page {target_page_id}: {page_name}")
    trace_out()
    return not is_error()
