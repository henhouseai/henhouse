...
@register_action('copy_audios')
@register_command('copy_audios')
def copy_audios() -> bool:
    trace_in()
    gateway = get_gateway()
    ...
    if not is_error():
        if not gateway.is_set('audio_id'):
            warn("No audio IDs provided")
            report_error("action", "Audio IDs are required")
    target_page_id: int = 0
    if not is_error():
        target_page_arg = gateway.get_arg('target_page') or gateway.get_arg('t_page')
        audio_id_str = gateway.get_arg('audio_id')
    ...
    # Parse comma-separated audio IDs
    audio_ids: List[int] = []
    if not is_error() and audio_id_str:
        try:
            audio_ids = [int(id_str.strip()) for id_str in audio_id_str.split(',') if id_str.strip()]
            if not audio_ids:
                warn("No valid audio IDs provided")
                report_error("action", "No valid audio IDs provided")
        except ValueError:
            warn(f"Invalid audio ID format: {audio_id_str}")
            report_error("action", "Audio IDs must be comma-separated numbers")
    ...
    if not is_error() and page is not None:
        log(f"Copying {len(audio_ids)} audio to target page {target_page_id}")
        success = page.copy_media_items("audio", audio_ids, target_rank=rank_int)
        if not success:
            warn(f"Failed to copy audio to target page {target_page_id}")
            report_error("action", f"Failed to copy audio to target page {target_page_id}")
    ...
    if not is_error() and updated_page is not None:
        ...
        response_data.update({
            "copied_audio_ids": audio_ids,
            "operation": "copy_audios"
        })
        ...
        if rank_int is not None:
            ...
            log(f"Successfully copied {len(audio_ids)} audio to target page {target_page_id} starting at rank {rank_int}")
        else:
            response_data.update({
                "rank_operation": "skipped"
            })
            log(f"Successfully copied {len(audio_ids)} audio to target page {target_page_id}")
