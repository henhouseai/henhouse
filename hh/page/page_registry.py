...
def _load_mcp_utils_for_page_class(PageClass: type) -> None:
    """Dynamically import mcp_utils module for a page class for HTTP backend only."""
    trace_in()
    gateway = get_gateway()
    if not gateway or gateway.backend != "http":
        # Only load mcp_utils for HTTP backend requests (MCP doesn't need hot cache population)
        trace_out()
        return
