...
@register_action('copy_file')
@register_command('copy_file')
def copy_file() -> bool:
    trace_in()
    gateway = get_gateway()
    ...
    if not is_error():
        ...
        try:
            ...
            warn(f"Invalid target page ID: {target_page_arg} or file ID: {file_id_arg}")
            report_error("action", "Target page ID and file ID must be numbers")
    ...
    if not is_error() and page is not None:
        log(f"Copying file {file_id} to target page {target_page_id}")
        success = page.copy_media_items("file", [file_id], target_rank=rank_int)
        if not success:
            warn(f"Failed to copy file {file_id} to target page {target_page_id}")
            report_error("action", f"Failed to copy file {file_id} to target page {target_page_id}")
    ...
    if not is_error() and updated_page is not None:
        ...
        if rank_int is not None:
            ...
            log(f"Successfully copied file {file_id} to target page {target_page_id} and set rank to {rank_int}")
        else:
            response_data.update({
                "rank_operation": "skipped"
            })
            log(f"Successfully copied file {file_id} to target page {target_page_id}")
