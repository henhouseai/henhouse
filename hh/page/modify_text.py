...
@register_action('modify_text')
@register_command('modify_text')
def modify_text() -> bool:
    ...
    if not is_error():
        ...
        # Handle the case where -text is passed as a flag (returns True)
        if new_text is True:
            new_text = ""
        try:
    ...
    if not is_error():
        ...
        log(f"Successfully modified page {page_id} text content with {total_children} children")
    trace_out()
    return not is_error()
