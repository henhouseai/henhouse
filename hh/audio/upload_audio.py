...
@register_action('upload_audio')
@register_command('upload_audio')
def upload_audio() -> bool:
    trace_in()
    gateway = get_gateway()
    ...
    if not is_error() and page is not None:
        ...
        while True:
            ...
            if not Path(file_path).exists():
                warn(f"Audio file {file_idx} not found: {file_path}")
                report_error("action", f"Audio file {file_idx} not found: {file_path}")
                break  # Stop on error
            
            if not os.access(file_path, os.R_OK):
                warn(f"Audio file {file_idx} not readable: {file_path}")
                report_error("action", f"Audio file {file_idx} not readable: {file_path}")
                break  # Stop on error
            ...
            log(f"Uploading audio {file_idx} '{file_caption}' to page {page_id} from temp file: {file_path}")
            new_audio_id = page.add_audio(file_path, file_caption)
            
            # Check for errors after upload
            if is_error():
                warn(f"Audio {file_idx} upload failed")
                break  # Stop on error
            
            if new_audio_id is None:
                warn(f"Audio {file_idx} upload failed: no audio ID returned")
                report_error("action", f"Audio {file_idx} upload failed")
                break  # Stop on error
            
            uploaded_audio_ids.append(new_audio_id)
            log(f"Successfully uploaded audio {file_idx} (ID: {new_audio_id}) to page {page_id}")
            
            file_idx += 1
    
    updated_page = None
    if not is_error():
        log(f"Reloading page {page_id} to show new audio")
    ...
    if not is_error() and updated_page is not None:
        ...
        log(f"Successfully uploaded {len(uploaded_audio_ids)} audio item(s) to page {page_id}: {page_name}")
