...
_dump_json()                  Line 198
_ensure_audio_cache_entry()   Line 201
...
class Audio:
    ...
    def __init__(self, audio_id: int):
        ...
        try:
            ...
            log(f"Loaded audio {audio_id}: {self.caption}")
            ...
            if not is_error():
                ...
                if last_modified_dt and cache_built_at_dt and cache_built_at_dt < last_modified_dt:
                    debug(f"Cache for audio {self.id} is stale (cache_built_at={cache_built_at_dt}, last_modified={last_modified_dt})")
                elif cache_built_at_dt is None:
                    debug(f"Cache for audio {self.id} does not exist (cache_built_at is NULL)")
                else:
                    ...
                    try:
                        ...
                        if cache_results:
                            ...
                            debug(f"Hydrated audio {self.id} from cache (built_at={self.cache_built_at})")
                    except Exception as e:
                        warn(f"Failed to hydrate audio {self.id} from cache: {e}")
    ...
    def _ensure_audio_cache_entry(self) -> bool:
        """Ensure cache entry exists in cache database. Only creates if missing."""
        trace_in()
        try:
            existing = self.gateway.conn.read_cache(
                "SELECT 1 FROM audio WHERE id = %s",
                (self.id,),
            )
        except Exception as exc:
            warn(f"Failed to check cache entry for audio {self.id}: {exc}")
            report_error("connection", f"Failed to verify cache for audio {self.id}")
            trace_out()
            return False
        ...
        try:
            self.gateway.conn.create_cache(
                """
                    INSERT INTO audio (id, instances, pages, metadata, cache_built_at)
                    VALUES (%s, %s, %s, %s, %s)
                """,
                (
                    self.id,
            ...
            debug(f"Created cache entry for audio {self.id}")
        except Exception as exc:
            warn(f"Failed to insert cache entry for audio {self.id}: {exc}")
            report_error("connection", f"Failed to create cache entry for audio {self.id}")
            trace_out()
            return False
    ...
    def _refresh_cached_audio(self) -> bool:
        ...
        if not self.gateway or not self.gateway.conn:
            ...
            trace_out()
            return False
        if not self._ensure_audio_cache_entry():
            debug(f"_refresh_cached_audio: Failed to ensure cache entry for audio {self.id}")
            trace_out()
            return False
        ...
        # This allows full audio hydration from cache database without main DB access
        main_db_metadata = {
        ...
        try:
            ...
            if affected == 0:
                debug(f"_refresh_cached_audio: UPDATE affected 0 rows for audio {self.id} - no change needed")
            ...
            if verify_check:
                ...
                warn(f"_refresh_cached_audio: Verification failed - cache entry not found after UPDATE")
            
            debug(f"Refreshed cache for audio {self.id}: rows={affected}, instances={len(self.instances) if self.instances else 0}, usage={len(self.cached_usage) if self.cached_usage else 0}")
        except Exception as exc:
