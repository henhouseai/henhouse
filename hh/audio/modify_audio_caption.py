...
@register_action('modify_audio_caption')
@register_command('modify_audio_caption')
def modify_audio_caption() -> bool:
    trace_in()
    gateway = get_gateway()
    ...
    if not is_error() and audio is not None:
        log(f"Modifying caption for audio {audio_id} to: {caption}")
        old_caption = audio.caption if audio.caption is not None else ""
        success = audio.modify_caption(caption)
        if not success:
            warn(f"Failed to modify caption for audio {audio_id}")
            report_error("action", f"Failed to modify caption for audio {audio_id}")
    ...
    if not is_error():
        ...
        log(f"Successfully modified caption for audio {audio_id}")
...
@register_parser('modify_audio_caption')
def modify_audio_caption_parser() -> bool:
    trace_in()
    gateway = get_gateway()
    ...
    try:
        ...
        lines = [render_header_block("l_modify_audio_caption_header")]
        
        table = TableData()
        table.add_row("modify_audio_caption_header", info="")
        ...
        lines.append(
            render_block(
                table,
                FieldConfig()
                .add_header("modify_audio_caption_header")
                .add_simple(["audio_id", "extra_data_old_caption", "extra_data_new_caption"]),
                block_type="rows",
