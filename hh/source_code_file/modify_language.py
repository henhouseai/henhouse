...
@register_action('modify_language')
@register_command('modify_language')
def modify_language() -> bool:
    trace_in()
    gateway = get_gateway()
    if not gateway:
    ...
    if not is_error():
        if not gateway.is_set('language'):
            warn("No new language provided")
            report_error("action", "New language is required")
    if not is_error():
        page_id_arg = gateway.get_arg('page_id') or gateway.get_arg('id')
        new_language = gateway.get_arg('language')
        # Handle the case where -language is passed as a flag (returns True)
        if new_language is True:
            new_language = ""
        try:
    ...
    if not is_error():
        ...
        if not success:
            warn("Page language modification failed")
            report_error("action", "Page language modification failed")
        ...
        log(f"Successfully modified page {page_id} language to '{new_language}' with {total_children} children")
    trace_out()
    return not is_error()
