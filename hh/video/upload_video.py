...
@register_action('upload_video')
@register_command('upload_video')
def upload_video() -> bool:
    ...
    if not is_error() and page is not None:
        ...
        while True:
            ...
            if not Path(file_path).exists():
                warn(f"Video file {file_idx} not found: {file_path}")
                report_error("action", f"Video file {file_idx} not found: {file_path}")
                break  # Stop on error
            
            if not os.access(file_path, os.R_OK):
                warn(f"Video file {file_idx} not readable: {file_path}")
                report_error("action", f"Video file {file_idx} not readable: {file_path}")
                break  # Stop on error
            ...
            log(f"Uploading video {file_idx} '{file_caption}' to page {page_id} from temp file: {file_path}")
            new_video_id = page.add_video(file_path, file_caption)
            
            # Check for errors after upload
            if is_error():
                warn(f"Video {file_idx} upload failed")
                break  # Stop on error
            
            if new_video_id is None:
                warn(f"Video {file_idx} upload failed: no video ID returned")
                report_error("action", f"Video {file_idx} upload failed")
                break  # Stop on error
            
            uploaded_video_ids.append(new_video_id)
            log(f"Successfully uploaded video {file_idx} (ID: {new_video_id}) to page {page_id}")
            
            file_idx += 1
    
    updated_page = None
    if not is_error():
        log(f"Reloading page {page_id} to show new video")
    ...
    if not is_error() and updated_page is not None:
        ...
        log(f"Successfully uploaded {len(uploaded_video_ids)} video item(s) to page {page_id}: {page_name}")
