...
from hh.work_docket.work_docket_method_registry import get_work_docket_method_registry
...
def _create_work_docket_wrapper_methods(cls):
    # Get the collected registry from all work_docket mixins
    method_registry = get_work_docket_method_registry()
    for method_name, config in method_registry.items():
        ...
        def make_dec_method(mixin_name):
            @decorator
            def work_docket_dec_method(self, conn: DatabaseConnection, *args, **kwargs):
                log(f"{mixin_name}")
                self.conn = conn
                mixin_method = getattr(super(cls, self), mixin_name)
                return mixin_method(*args, **kwargs)
            return work_docket_dec_method
        # Create connection version with closure
        def make_conn_method(mixin_name):
            def work_docket_conn_method(self, *args, **kwargs):
                log(f"{mixin_name}")
                mixin_method = getattr(super(cls, self), mixin_name)
                return mixin_method(*args, **kwargs)
            return work_docket_conn_method
        ...
        dec_method_name = f'work_docket_dec_{method_name}'
        conn_method_name = f'work_docket_conn_{method_name}'
...
@_create_work_docket_wrapper_methods
