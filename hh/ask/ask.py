...
class Ask(WorkPage):
    ...
    def getChildrenOf(cls, parent_id: int, view_type: str = 'table') -> List[Dict[str, Any]]:
        ...
        # Query for ask pages (not tasks) - this is for finding ask children of a parent
        query = """
            SELECT pages.id
            FROM pages
            WHERE pages.parent = %s
        ...
        if view_type == 'table':
            ...
            log(f"Loaded {len(children_data)} children for class 'ask' (table format)")
        else:  # view_type == 'tile'
            if results:
                for row in results:
                    child_page = get_page(page_id=row['id'])
                    if child_page:
                        ...
                        children_data.append(child_data)
            log(f"Loaded {len(children_data)} children for class 'ask' (tile format)")
