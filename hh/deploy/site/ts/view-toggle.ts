...
class ViewToggle {
  ...
  private initialize(): void {
    ...
    document.addEventListener('click', (e) => {
      ...
      const toggleLink = target.closest('a[class*="update"][class*="View_"]') as HTMLElement;
      if (!toggleLink) {
        return;
      }
  ...
  private async handleToggleClick(linkElement: HTMLElement): Promise<void> {
    try {
      ...
      const firstLink = allLinks[0] as HTMLElement;
      ...
      const context: ViewToggleContext = {
        ...
        section,
        className,
        viewType: viewType as 'table' | 'tile', // Type assertion for compatibility
        isInOverlay,
        linkElement
      };
