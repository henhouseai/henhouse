...
export class PageActionsFiles {
   ...
   */
  async copy_files_app(this: PageData, rpc: any): Promise<void> {
      ...
      const browser = new Browser({
        ...
        onSubmit: async (result: any) => {
          if (typeof result === 'object' && 'fileInstances' in result) {
            const fileResult = result as { fileIds: number[]; fileInstances: Array<{ file_id: number; source_page_id: number; source_rank: number }> };
              ...
              try {
                ...
                if (response?.data && browserOverlay) {
                  ...
                  browserOverlay.setState({
                    messages: [...currentMessages, { type: 'success' as const, text: `Copied files from page ${sourcePageId}` }]
                  });
                }
              } catch (error) {
                if (browserOverlay) {
                  ...
                  browserOverlay.setState({
                    messages: [...currentMessages, { type: 'error' as const, text: `Failed to copy files from page ${sourcePageId}: ${msg}` }]
                  });
                }
   ...
   */
  async move_files_app(this: PageData, rpc: any): Promise<void> {
    const pageId = this.id;
    if (!pageId) {
      alert('No page ID found');
      return;
            ...
            for (const [sourcePageIdStr, ranks] of Object.entries(bySource)) {
              ...
              try {
                ...
                if (response?.data && browserOverlay) {
                  const currentMessages = (browserOverlay as any)['state'].messages || [];
                  browserOverlay.setState({
                    messages: [...currentMessages, { type: 'success' as const, text: `Moved files from page ${sourcePageId}` }]
                  });
                }
              } catch (error) {
                if (browserOverlay) {
                  ...
                  const msg = error instanceof Error ? error.message : String(error);
                  browserOverlay.setState({
                    messages: [...currentMessages, { type: 'error' as const, text: `Failed to move files from page ${sourcePageId}: ${msg}` }]
                  });
                }
                if (error && (error as any).debug?.entries?.length) {
                  const { handleRPCResponseWithDebug } = await import('./debug-helper.js');
   ...
   */
  async delete_files_app(this: PageData, rpc: any): Promise<void> {
    const pageId = this.id;
    if (!pageId) {
      alert('No page ID found');
      return;
            ...
            for (const instance of fileResult.fileInstances) {
              ...
              try {
                ...
                if (response?.data && browserOverlay) {
                  const currentMessages = (browserOverlay as any)['state'].messages || [];
                  browserOverlay.setState({
                    messages: [...currentMessages, { type: 'success' as const, text: `Deleted file ${instance.file_id} from page ${instance.source_page_id}` }]
                  });
                }
              } catch (error) {
                if (browserOverlay) {
                  ...
                  const msg = error instanceof Error ? error.message : String(error);
                  browserOverlay.setState({
                    messages: [...currentMessages, { type: 'error' as const, text: `Failed to delete file ${instance.file_id}: ${msg}` }]
                  });
                }
                if (error && (error as any).debug?.entries?.length) {
                  const { handleRPCResponseWithDebug } = await import('./debug-helper.js');
   ...
   */
  async sort_files_app(this: PageData, rpc: any): Promise<void> {
    const pageId = this.id;
    if (!pageId) {
      alert('No page ID found');
      return;
