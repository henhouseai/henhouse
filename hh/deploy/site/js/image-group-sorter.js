...
export class ImageGroupSorter {
    ...
    prepareForSorting() {
        ...
        listItems.forEach((li) => {
            ...
            if (link) {
                ...
                if (dataImageId) {
                    const imageId = parseInt(dataImageId.getAttribute('data-image-id') || '0', 10);
                    if (imageId > 0) {
                        li.setAttribute('data-id', imageId.toString());
                    }
                }
    ...
    async handleSubmit() {
        ...
        if (desiredOrder.length === 0) {
            ...
            this.overlay.setState({
                messages: [...currentMessages, { type: 'error', text: 'No images found to sort' }]
            });
        ...
        while (true) {
            ...
            if (iterationCount > maxIterations) {
                const currentMessages = this.overlay['state'].messages || [];
                this.overlay.setState({
                    messages: [...currentMessages, { type: 'error', text: `Sorting mechanism failed: exceeded maximum iterations (${maxIterations}). Could not find optimal sort.` }]
                });
            ...
            try {
                ...
                const currentMessages = this.overlay['state'].messages || [];
                this.overlay.setState({
                    messages: [...currentMessages, { type: 'success', text: `Successfully moved image ${imageToMove.imageId} to rank ${imageToMove.desiredRank}` }]
                });
            }
            catch (error) {
                ...
                const newMessages = [{ type: 'error', text: `Failed to move image ${imageToMove.imageId} to rank ${imageToMove.desiredRank}: ${errorMsg}` }];
    ...
    buildDesiredRanks() {
        ...
        listItems.forEach((item, index) => {
            ...
            const liDataId = item.getAttribute('data-id');
            if (liDataId) {
