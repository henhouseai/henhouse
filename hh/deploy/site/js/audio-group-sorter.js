...
export class AudioGroupSorter {
    constructor(pageId) {
        this.overlay = null;
        this.tableHtml = '';
        this.currentRanks = new Map(); // audioId -> rank
        this.desiredRanks = new Map(); // audioId -> desired rank
        ...
        rows.forEach(row => {
            ...
            if (cells.length >= 2) {
                ...
                if (!Number.isNaN(audioId)) {
                    row.setAttribute('data-id', audioId.toString());
                }
            }
        });
    }
    ...
    async handleSubmit() {
        ...
        if (desiredOrder.length === 0) {
            ...
            this.overlay.setState({
                messages: [...currentMessages, { type: 'error', text: 'No audio files found to sort' }]
            });
        ...
        while (true) {
            ...
            try {
                ...
                const currentMessages = this.overlay['state'].messages || [];
                this.overlay.setState({
                    messages: [...currentMessages, { type: 'success', text: `Moved audio file ${audioToMove.audioId} to rank ${audioToMove.desiredRank}` }]
                });
            }
            catch (error) {
                const currentMessages = this.overlay['state'].messages || [];
                ...
                this.overlay.setState({
                    messages: [...currentMessages, { type: 'error', text: `Failed to move audio file ${audioToMove.audioId}: ${msg}` }]
                });
