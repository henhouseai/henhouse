...
export class VideoGroupSorter {
    constructor(pageId) {
        this.overlay = null;
        this.tableHtml = '';
        this.currentRanks = new Map(); // videoId -> rank
        this.desiredRanks = new Map(); // videoId -> desired rank
        ...
        rows.forEach(row => {
            ...
            if (cells.length >= 2) {
                ...
                if (!Number.isNaN(videoId)) {
                    row.setAttribute('data-id', videoId.toString());
                }
            }
        });
    }
    ...
    async handleSubmit() {
        ...
        if (desiredOrder.length === 0) {
            ...
            this.overlay.setState({
                messages: [...currentMessages, { type: 'error', text: 'No video files found to sort' }]
            });
        ...
        while (true) {
            ...
            try {
                ...
                const currentMessages = this.overlay['state'].messages || [];
                this.overlay.setState({
                    messages: [...currentMessages, { type: 'success', text: `Moved video file ${videoToMove.videoId} to rank ${videoToMove.desiredRank}` }]
                });
            }
            catch (error) {
                const currentMessages = this.overlay['state'].messages || [];
                ...
                this.overlay.setState({
                    messages: [...currentMessages, { type: 'error', text: `Failed to move video file ${videoToMove.videoId}: ${msg}` }]
                });
