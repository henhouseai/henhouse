                            ...
                            try {
                                ...
                                if (response?.data && browserOverlay) {
                                    ...
                                    browserOverlay.setState({
                                        messages: [...currentMessages, { type: 'success', text: `Copied audio files from page ${sourcePageId}` }]
                                    });
                                }
                            }
                            catch (error) {
                                if (browserOverlay) {
                                    ...
                                    browserOverlay.setState({
                                        messages: [...currentMessages, { type: 'error', text: `Failed to copy audio files from page ${sourcePageId}: ${msg}` }]
                                    });
                                }
                        ...
                        for (const [sourcePageIdStr, ranks] of Object.entries(bySource)) {
                            ...
                            try {
                                ...
                                if (response?.data && browserOverlay) {
                                    const currentMessages = browserOverlay['state'].messages || [];
                                    browserOverlay.setState({
                                        messages: [...currentMessages, { type: 'success', text: `Moved audio files from page ${sourcePageId}` }]
                                    });
                                }
                            }
                            catch (error) {
                                if (browserOverlay) {
                                    ...
                                    const msg = error instanceof Error ? error.message : String(error);
                                    browserOverlay.setState({
                                        messages: [...currentMessages, { type: 'error', text: `Failed to move audio files from page ${sourcePageId}: ${msg}` }]
                                    });
                                }
                                if (error && error.debug?.entries?.length) {
                                    const { handleRPCResponseWithDebug } = await import('./debug-helper.js');
                        ...
                        for (const instance of audioResult.fileInstances) {
                            ...
                            try {
                                ...
                                if (response?.data && browserOverlay) {
                                    const currentMessages = browserOverlay['state'].messages || [];
                                    browserOverlay.setState({
                                        messages: [...currentMessages, { type: 'success', text: `Deleted audio file ${instance.file_id} from page ${instance.source_page_id}` }]
                                    });
                                }
                            }
                            catch (error) {
                                if (browserOverlay) {
                                    ...
                                    const msg = error instanceof Error ? error.message : String(error);
                                    browserOverlay.setState({
                                        messages: [...currentMessages, { type: 'error', text: `Failed to delete audio file ${instance.file_id}: ${msg}` }]
                                    });
                                }
                                if (error && error.debug?.entries?.length) {
                                    const { handleRPCResponseWithDebug } = await import('./debug-helper.js');
