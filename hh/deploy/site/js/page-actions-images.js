                        ...
                        for (const imageId of imageResult.imageIds) {
                            try {
                                ...
                                if (response && response.data) {
                                    ...
                                    if (browserOverlay) {
                                        ...
                                        browserOverlay.setState({
                                            messages: [...currentMessages, { type: 'success', text: `Successfully copied image ${imageId}` }]
                                        });
                                    }
                                }
                            }
                                ...
                                if (browserOverlay) {
                                    ...
                                    const newMessages = [{ type: 'error', text: `Failed to copy image ${imageId}: ${errorMsg}` }];
                            ...
                            try {
                                ...
                                if (response && response.data) {
                                    ...
                                    if (browserOverlay) {
                                        const currentMessages = browserOverlay['state'].messages || [];
                                        browserOverlay.setState({
                                            messages: [...currentMessages, { type: 'success', text: `Successfully moved image(s) ${imageIds} from page ${sourcePageId}` }]
                                        });
                                    }
                                }
                                else {
                                ...
                                if (browserOverlay) {
                                    ...
                                    const detailedErrors = (error && typeof error === 'object' && 'errors' in error && Array.isArray(error.errors))
                                        ...
                                        : [];
                                    // Add main error message
                                    const newMessages = [{ type: 'error', text: `Failed to move images from page ${sourcePageId}: ${errorMsg}` }];
                                    // Add detailed errors if available
                                    if (detailedErrors.length > 0) {
                                        detailedErrors.forEach((err) => {
                                            newMessages.push({
                                ...
                                try {
                                    ...
                                    if (response && response.data) {
                                        ...
                                        if (browserOverlay) {
                                            const currentMessages = browserOverlay['state'].messages || [];
                                            browserOverlay.setState({
                                                messages: [...currentMessages, { type: 'success', text: `Successfully removed image ${instance.image_id} (rank ${instance.source_rank}) from page ${sourcePageId}` }]
                                            });
                                        }
                                    }
                                    else {
                                    ...
                                    if (browserOverlay) {
                                        ...
                                        const detailedErrors = (error && typeof error === 'object' && 'errors' in error && Array.isArray(error.errors))
                                            ...
                                            : [];
                                        // Add main error message
                                        const newMessages = [{ type: 'error', text: `Failed to remove image ${instance.image_id} (rank ${instance.source_rank}) from page ${sourcePageId}: ${errorMsg}` }];
                                        // Add detailed errors if available
                                        if (detailedErrors.length > 0) {
                                            detailedErrors.forEach((err) => {
                                                newMessages.push({
