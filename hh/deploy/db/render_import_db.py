...
def render_import_db_section(source_data, lines):
    trace_in()
    try:
        db_data = TableData()
        db_data.add_row(
        ...
        import_file = source_data.get('import_file', 'Unknown')
        db_data.add_row(
            'import_file',
            value=import_file
        )
        import_size = source_data.get('import_size', 0)
        size_mb = import_size / (1024 * 1024)
        db_data.add_row(
            'import_size',
        ...
        if table_info:
            lines.append(render_block(
                ...
                FieldConfig()
                    ...
                    .add_simple(['import_file', 'import_size', 'table_count', 'table_info', 'name', 'row_count']),
                table_overrides={'margin_l': 4}
            ))
    except Exception as e:
        warn(f"Failed to render database import section: {str(e)}")
    finally:
        trace_out()

@register_parser('import_db')
def import_db() -> bool:
    trace_in()
    gateway = get_gateway()
    if not gateway:
