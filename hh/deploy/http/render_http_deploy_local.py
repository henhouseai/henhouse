...
def render_http_deploy_local_section(source_data: Dict[str, Union[str, int, bool]], lines: List[str]) -> None:
    trace_in()
    gateway = get_gateway()
    if not gateway:
        warn("No gateway available in render_http_deploy_local_section")
        trace_out()
        return
    ...
    if not gateway.is_no(block):
        ...
        log(f"Rendering HTTP deploy local section for: {domain}")
        ...
        lines.append(render_block(
            ...
            FieldConfig()
                ...
                .add_simple(['port', 'http_config_path', 'bind_ips', 'nginx_active', 'main_domain', 'subdomain', 'js_whitelist', 'css_whitelist', 'misc_whitelist', 'context_whitelist']),
...
@register_parser('http_deploy_local')
def http_deploy_local() -> bool:
    ...
    lines = []
    lines.append(render_header_block('l_http_deploy_header'))
    source_data = get_data(json_data if json_data is not None else {})
    log(f"Processing HTTP deploy local data successfully")
    
    render_http_deploy_local_section(source_data, lines)
