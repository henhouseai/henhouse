...
def _manifest_has_site(project_name: str, domain: str) -> bool:
    cfg_path = _install_config_path(project_name)
    if not cfg_path.exists():
        return False
    parser = configparser.ConfigParser()
    parser.read(cfg_path)
    return parser.has_section("manifest_sites") and parser.has_option("manifest_sites", domain)

def validate_nginx_config_for_deletion(config_file: Path, force: bool = False) -> bool:
    """Validate that an nginx config file exists."""
    trace_in()
    try:
        ...
        if force:
            log(f"Force flag enabled - skipping checks for {config_file}")
            trace_out()
            return True
        
        log(f"Config file {config_file} exists")
        trace_out()
        return True
...
def http_remove() -> bool:
    ...
    # Require manifest entry unless force
    if not force and not _manifest_has_site(project_name, domain):
        warn(f"Domain {domain} not found in manifest_sites; refusing removal (use force to override).")
        trace_out()
        return False
