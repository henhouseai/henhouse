...
def render_http_deploy_section(source_data: Dict[str, Union[str, int, bool]], lines: List[str]) -> None:
    trace_in()
    gateway = get_gateway()
    if not gateway:
        warn("No gateway available in render_http_deploy_section")
        trace_out()
        return
    ...
    if not gateway.is_no(block):
        ...
        log(f"Rendering HTTP deploy section for: {domain}")
        ...
        lines.append(render_block(
            ...
            FieldConfig()
                ...
                .add_simple(['port', 'http_config_path', 'nginx_active', 'main_domain', 'subdomain', 'js_whitelist', 'css_whitelist', 'misc_whitelist', 'context_whitelist']),
...
@register_parser('http_deploy')
def http_deploy() -> bool:
    trace_in()
    gateway = get_gateway()
    if not gateway:
    ...
    log(f"Processing HTTP deploy data successfully")
    
    render_http_deploy_section(source_data, lines)
