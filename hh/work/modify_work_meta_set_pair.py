...
@register_action('modify_work_meta_set_pair')
@register_command('modify_work_meta_set_pair')
def modify_work_meta_set_pair() -> bool:
    trace_in()
    gateway = get_gateway()
    if not gateway:
    ...
    if not is_error():
        if not gateway.is_set('key'):
            warn("No key provided")
            report_error("action", "Key is required")
    if not is_error():
        if not gateway.is_set('value'):
            warn("No value provided")
            report_error("action", "Value is required")
    if not is_error():
        page_id_arg = gateway.get_arg('page_id') or gateway.get_arg('id')
        key = gateway.get_arg('key')
        value = gateway.get_arg('value')
        try:
    ...
    if not is_error():
        log(f"Setting meta key '{key}' for page {page_id}")
        success = page.modify_work_meta_set_pair(key, value)
        if not success:
            warn("Page meta set pair failed")
            report_error("action", "Page meta set pair failed")
        ...
        log(f"Successfully set meta key '{key}' for page {page_id} with {total_children} children")
    trace_out()
    return not is_error()
