...
def modify_work_meta() -> bool:
    ...
    if not is_error():
        if not gateway.is_set('meta'):
            warn("No new meta provided")
            report_error("action", "New meta is required")
    if not is_error():
        page_id_arg = gateway.get_arg('page_id') or gateway.get_arg('id')
        new_meta = gateway.get_arg('meta')
    ...
    if not is_error():
        log(f"Modifying page {page_id} meta")
        success = page.modify_work_meta(new_meta)
        if not success:
            warn("Page meta modification failed")
            report_error("action", "Page meta modification failed")
        ...
        log(f"Successfully modified page {page_id} meta with {total_children} children")
    trace_out()
    return not is_error()
